...
if [ ! -f ${PATCH_FILE} ]; then
	echo "patch file for ISTGT not found"
	exit 1
fi

if [ $# -ne 1 ]; then
	echo "ISTGT source directory not provided"
	exit 1
fi

istgt_dir=$1
cd $istgt_dir
git checkout ${ISTGT_TAG}
if [ $? -ne 0 ]; then
...
patch -p1 -N --dry-run --silent< ${CUR_DIR}/${PATCH_FILE} 2>&1>/dev/null
if [ $? -eq 0 ]; then
